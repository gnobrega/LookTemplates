package com.damidia {
	...
	public class TextEffect
	{
		...
		static public function aplicaDivisao(stage, texto:TextField, maximoLinhas, bg:Class, strTexto:String):void
		{
			...
			var distanciaDeBaixo:Number = stage.stageHeight - (texto.y + texto.height);
			...
			linhas[0].setTextFormat(formatacao);
			
			var diferenca:uint = Math.floor(linhas[0].height);
			
			for(var idx:uint = linhas.length, atual:uint=1; --idx >= 0; atual++)
			{
				var linhaAtual:TextField = linhas[idx];
				
				linhaAtual.setTextFormat(formatacao);
				
				linhaAtual.y = stage.stageHeight - (distanciaDeBaixo + (diferenca * atual));
